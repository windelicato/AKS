...
#define DEFAULT_GPIO_SLAVE_ADDRESSES {0x24,0x27}
...
int setup_i2c_LED_Drivers() {
	...
	for(i = 0; i<DEFAULT_NUM_LED_DRIVER_IC; i++) {
		...
		if(write(i2c_bus,ledout_init,sizeof(ledout_init))!=sizeof(ledout_init)) {
			//TEST
			printf("Error writing to LED Driver LED_OUT registers!\n");
		...
		if(write(i2c_bus,irefall_init,sizeof(irefall_init))!=sizeof(irefall_init)) {
			//TEST
			printf("Error writing to LED Driver IREF_ALL register!\n");
	...
	#ifdef DEBUG
	//TEST
	printf("Finished setup_i2c_LED_Drivers()...\n");
	#endif
...
int i2c_setGPIOPins(int chip, char* pins) {
	...
	//TEST
//	printf("Success writing multiple bytes to i2c device.\n");
	return 0;
}
